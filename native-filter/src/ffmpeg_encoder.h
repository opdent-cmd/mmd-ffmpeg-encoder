...
private:
    ...
    void    AppendCodecArgs(std::wstring& cmd, const std::wstring& codec,
                            const std::wstring& preset, bool bottomUp);
    HRESULT StartFFmpeg();
    void    SelectPortableCodec();
    void    StopFFmpeg();
    void    ResolveOutputPaths();
    void    PostProcessOutputs();
    bool    MergeAudio();
    bool    RunCommand(const std::wstring& cmd, DWORD timeoutMs);
    ...
    std::wstring m_extra;
    std::wstring m_container;
    std::wstring m_containerPath;
    std::wstring m_alphaFormat;
